...
static void amiibo_scene_storage_list_menu_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    case STORAGE_LIST_MENU_FORMAT: {
        ...
        mui_msg_box_set_header(app->p_msg_box, "格式化存儲");
        mui_msg_box_set_message(app->p_msg_box, "將刪除所有數據。\n確認格式化?");
        mui_msg_box_set_btn_text(app->p_msg_box, "確定", NULL, "取消");
...
void amiibo_scene_storage_list_menu_on_enter(void *user_data) {
    ...
    if (res == VFS_OK) {
        ...
        #define mount_str "===已掛載[LFS]==="
        #else
        #define mount_str "===已掛載[FFS]==="
        #endif
        mui_list_view_add_item(app->p_list_view, 0xe1ca,
                               stat.avaliable ? mount_str : "=====未挂载=====", (void *)-1);
        if (stat.avaliable) {
            snprintf(txt, sizeof(txt), "總空間: %d kB", stat.total_bytes / 1024);
            mui_list_view_add_item(app->p_list_view, 0xe1cb, txt, (void *)-1);
            snprintf(txt, sizeof(txt), "可用空間: %d kB", stat.free_bytes / 1024);
    ...
    mui_list_view_add_item(app->p_list_view, ICON_HOME, "返回主菜單", (void *)STORAGE_LIST_MENU_BACK_MAIN_MENU);
